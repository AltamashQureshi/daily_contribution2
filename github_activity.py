"""
╔══════════════════════════════════════════════════════╗
║        GitHub Activity Graph Enhancer v1.0           ║
    ...
    for attempt in range(1, API_MAX_RETRIES + 1):
        try:
            ...
            if r.status_code in (429, 403):
                wait = int(r.headers.get("Retry-After", API_RETRY_DELAY * attempt))
...
# FIX 3a: Dedicated merge call — GitHub returns 204 No Content on success,
# not 200+JSON, so the old api_post() always saw it as a failure.
...
def main():
    ...
    while current <= END_DATE:
        ...
        # ── Issues ────────────────────────────────────────────────
        if random.random() < AVG_ISSUES_PER_WEEK / 7:
            ok = create_issue(
        ...
        # ── PRs ───────────────────────────────────────────────────
        if random.random() < AVG_PRS_PER_WEEK / 7:
            ok = create_pr(
